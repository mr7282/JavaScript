...
const snake = {
  ...
  // Отображает текущий счет, проверяет если нажата ли кнопка Новая игра ****************************************************************************************************
  showScore(res) {
      const bordElem = document.getElementById('board');
      if (res === 0) {
        bordElem.innerHTML = "Счёт";
      } else {
        bordElem.innerHTML = `${this.getScore()}`;
      }
  },
...
const game = {
   ...
   */
  reset() {
    // Присваивает начальное значения текстового поля board ******************************************************************************************************************
    this.snake.showScore(0);
